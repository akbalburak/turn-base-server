...
namespace TurnBase.Server.ServerModels
{
    public class SocketUser : BaseSocketUser
    {
        ...
        public Battle.Core.BattleItem CurrentBattle { get; private set; }
        ...
        public SocketUser(System.Net.Sockets.Socket socket) : base(socket)
        {
            this.TempUniqueUserID = Guid.NewGuid();
            this.User = new SocketUserData();

            this._waitingRequests = new List<SocketRequest>();
            this._waitingResponses = new List<Tuple<SocketRequest, byte[]>>();

            this._unExpectedNotReceivedResponses = new List<SocketResponse>();
        ...
        public void SetBattle(Battle.Core.BattleItem battle)
        {
        ...
        public void ActionExecuter(SocketRequest request)
        {
            try
            {
                ...
                if (!response.IsSuccess && this.User != null)
        ...
        public void SendToClient(byte[] data)
        {
            try
            {
                if (!IsDisposed && this.Socket != null && this.Socket.Connected)
